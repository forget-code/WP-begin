...
// 形式名称
function custom_post_format( $safe_text ) {
	if ( $safe_text == '引语' )
		return '软件';
	return $safe_text;
}
...
<select id="sc_select">
	...
	<option value="[slide h=高度]插入图片[/slide]">添加幻灯</option>
...
function begin_register_tinymce_button( $buttons ) {
	array_push( $buttons, "down", "url", "videos", "img", "slide", "reply", "login", "password", "addcode", "addfolding", "field", "ad" );
	return $buttons;
}
...
function custom_user_contact($user_contactmethods){
	//去掉默认联系方式
