<div id="login">
	...
	<div id="login-tab" class="login-tab-product">
	    <h2 class="login-tab-hd">
			...
			<?php } ?>
			<span class="login-tab-hd-con"><a href="javascript:"><?php _e( '找回密码', 'begin' ); ?></a></span>
	    </h2>
	...
	<div class="sidebox">
		...
		<div class="usericon">
			<?php global $userdata; get_currentuserinfo(); echo get_avatar($userdata->ID, 80); ?>
		</div>
		<div class="userinfo">
			<p>
				<?php if (current_user_can('manage_options')) { 
				echo '<a href="' . admin_url() . '" target="_blank">'.sprintf(__( '管理站点', 'begin' )).'</a>'; } else { 
				echo '<a href="' . admin_url('profile.php') . '" target="_blank">'.sprintf(__( '用户中心', 'begin' )).'</a>'; } ?>
